...
const EVENT_COLORS: Record<string, string> = {
  meeting: '#3b82f6', appointment: '#8b5cf6', call: '#10b981',
  deadline: '#ef4444', event: '#f59e0b', reminder: '#6b7280',
}
...
export default function AgentClient({ agent }: { agent: Record<string, unknown> }) {
  ...
  const loadRecentRuns = async () => {
    const { data } = await supabase.from('automation_runs').select('*').eq('business_agent_id', agent.id as string).order('created_at', { ascending: false }).limit(5)
    setRecentRuns(data || [])
  }
  ...
  const startAction = (action: typeof QUICK_ACTIONS[0]) => {
    ...
    if (action.prompt) {
      ...
      setTimeout(() => { inputRef.current?.focus(); inputRef.current?.setSelectionRange(action.prompt.length, action.prompt.length) }, 100)
    } else {
      setInput('')
  ...
  const sendMessage = async () => {
    ...
    setRunning(true)
    try {
      const response = await fetch('/api/agent-chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: input, agent, history: messages }) })
      const data = await response.json()
      setMessages(prev => [...prev, { role: 'assistant', content: data.reply, timestamp: new Date().toISOString(), emailSent: data.emailSent, documentId: data.documentId, documentType: data.documentType, invoiceHTML: data.invoiceHTML, calendarEvent: data.calendarEvent }])
      if (data.calendarEvent) loadCalendarEvents()
      await supabase.from('automation_runs').insert({ business_agent_id: agent.id as string, automation_type: 'chat', input, output: data.reply })
      loadRecentRuns()
    } catch {
      ...
      <main className="app-main">
        ...
        <div className="agent-content" style={{ width: '100%', padding: '40px 48px' }}>
          ...
          {view === 'home' && (
            <div>
              {/* Banner */}
              <div style={{ width: '100%', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '36px 40px', marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                  <div style={{ width: 64, height: 64, borderRadius: 16, background: 'var(--bg4)', border: '1px solid var(--border2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--sidebar-font)', fontSize: 28, color: 'var(--fg)', fontWeight: 700, flexShrink: 0 }}>
                    {(agent.agent_name as string)?.[0]}
                  </div>
                  <div>
                    <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.5px', marginBottom: 6, color: 'var(--fg)', fontFamily: 'var(--sidebar-font)' }}>{agent.agent_name as string}</div>
                    <div style={{ fontSize: 15, color: 'var(--fg3)', fontFamily: 'var(--sidebar-font)' }}>{agent.business_name as string} · {agent.industry as string}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                  <button onClick={() => { if (messages.length === 0) setMessages([{ role: 'assistant', content: `Hi! I'm ${agent.agent_name as string}, your AI assistant for ${agent.business_name as string}. What do you need done?`, timestamp: new Date().toISOString() }]); setView('chat') }} className="btn btn-accent" style={{ height: 44, padding: '0 28px', fontSize: 15, fontWeight: 600, fontFamily: 'var(--sidebar-font)' }}>
                    Start chatting →
                  </button>
                  <button onClick={() => window.open(`/portal/${agent.id as string}`, '_blank')} className="btn btn-outline" style={{ height: 44, padding: '0 20px', fontSize: 14, fontFamily: 'var(--sidebar-font)', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button onClick={copyPortalLink} className="btn btn-outline" style={{ height: 44, padding: '0 20px', fontSize: 14, fontFamily: 'var(--sidebar-font)', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                    {portalCopied ? <><svg width="14" height="14" fill="none" stroke="var(--accent)" strokeWidth="2" viewBox="0 0 24 24"><path d="M20 6 9 17l-5-5" /></svg><span style={{ color: 'var(--accent)' }}>Copied!</span></> : <><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></svg>Copy portal link</>}
                  </button>
                </div>
              </div>
              ...
              <div className="agent-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 40 }}>
                {[
                  ...
                  <div key={stat.label} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '24px 28px', display: 'flex', alignItems: 'center', gap: 18 }}>
                    <div style={{ width: 48, height: 48, borderRadius: 12, background: 'var(--bg3)', border: '1px solid var(--border2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: stat.color, flexShrink: 0 }}>{stat.icon}</div>
                    <div>
                      <div style={{ fontFamily: 'var(--sidebar-font)', fontSize: 12, color: 'var(--fg3)', marginBottom: 6, fontWeight: 500 }}>{stat.label}</div>
                      <div style={{ fontSize: 22, fontWeight: 700, color: stat.color, lineHeight: 1, fontFamily: 'var(--sidebar-font)' }}>{stat.value}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Portal info card */}
              <div style={{ background: 'linear-gradient(135deg, #0a1200 0%, #0d1a00 100%)', border: '1px solid rgba(200,241,53,0.15)', borderRadius: 12, padding: '24px 28px', marginBottom: 36, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
                <div>
                  <div style={{ fontFamily: 'var(--sidebar-font)', fontSize: 14, fontWeight: 600, color: 'var(--accent)', marginBottom: 6 }}>Customer Portal</div>
                  <div style={{ fontFamily: 'var(--sidebar-font)', fontSize: 13, color: 'var(--fg3)', marginBottom: 8 }}>Share this link with your customers so they can chat with your AI agent directly.</div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => window.open(`/portal/${agent.id as string}`, '_blank')} className="btn btn-outline btn-sm" style={{ fontFamily: 'var(--sidebar-font)', fontSize: 12, display: 'inline-flex', alignItems: 'center', gap: 6, borderColor: 'rgba(200,241,53,0.3)', color: 'var(--accent)' }}>
                  ...
                  <button onClick={copyPortalLink} className="btn btn-accent btn-sm" style={{ fontFamily: 'var(--sidebar-font)', fontSize: 12 }}>
                    ...
                    {upcomingEvents.map(event => (
                      <div key={event.id} style={{ flexShrink: 0, padding: '14px 18px', background: 'var(--bg2)', borderRadius: 10, border: '1px solid var(--border)', borderLeft: `3px solid ${EVENT_COLORS[event.event_type] || '#6b7280'}`, minWidth: 180 }}>
                        ...
                        <div style={{ marginTop: 8 }}>
                          <span style={{ fontFamily: 'var(--mono)', fontSize: 9, padding: '2px 6px', borderRadius: 4, background: `${EVENT_COLORS[event.event_type] || '#6b7280'}22`, color: EVENT_COLORS[event.event_type] || '#6b7280', border: `1px solid ${EVENT_COLORS[event.event_type] || '#6b7280'}44` }}>{event.event_type}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                    {recentRuns.map((run, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', borderBottom: i < recentRuns.length - 1 ? '1px solid var(--border)' : 'none', cursor: 'pointer', transition: 'background 0.1s' }} onClick={() => { setView('chat'); setMessages([{ role: 'user', content: run.input as string, timestamp: run.created_at as string }, { role: 'assistant', content: run.output as string, timestamp: run.created_at as string }]) }} onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = 'var(--bg3)'} onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}>
                        <div style={{ overflow: 'hidden', flex: 1 }}>
                          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'var(--sidebar-font)' }}>{(run.input as string)?.slice(0, 70)}{(run.input as string)?.length > 70 ? '...' : ''}</div>
                          <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg3)' }}>{run.automation_type as string}</div>
                        </div>
                        <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg3)', flexShrink: 0, marginLeft: 20 }}>{timeAgo(run.created_at as string)}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {view === 'chat' && (
            <div className="chat-container" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 160px)' }}>
                ...
                {messages.map((msg, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '75%', background: msg.role === 'user' ? 'var(--fg)' : 'var(--bg2)', color: msg.role === 'user' ? 'var(--bg)' : 'var(--fg)', border: `1px solid ${msg.role === 'user' ? 'var(--fg)' : 'var(--border)'}`, borderRadius: msg.role === 'user' ? '14px 14px 4px 14px' : '14px 14px 14px 4px', padding: '12px 16px' }}>
                      {msg.role === 'assistant' && <div style={{ fontFamily: 'var(--sidebar-font)', fontSize: 11, color: 'var(--accent)', marginBottom: 8, fontWeight: 700, letterSpacing: 0.5 }}>{agent.agent_name as string}</div>}
                      <div style={{ fontSize: 14, lineHeight: 1.7, whiteSpace: 'pre-wrap', fontFamily: 'var(--sans)' }}>{msg.content}</div>
                      {msg.emailSent && (
                      ...
                      {msg.documentId && (
                        <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          <button onClick={() => viewDocument(msg.documentId!, msg.documentType!, msg.invoiceHTML)} style={{ fontFamily: 'var(--sidebar-font)', fontSize: 12, padding: '7px 14px', background: 'var(--fg)', color: 'var(--bg)', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <button onClick={() => { const win = window.open('', '_blank'); if (win && msg.invoiceHTML) { win.document.write(msg.invoiceHTML); win.document.close(); win.print() } }} style={{ fontFamily: 'var(--sidebar-font)', fontSize: 12, padding: '7px 14px', background: '#c8f135', color: '#0a0a0a', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--fg3)', marginTop: 8, opacity: 0.6 }}>{new Date(msg.timestamp).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</div>
                    </div>
                  </div>
                ))}
                {running && (
                  <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '14px 14px 14px 4px', padding: '12px 16px' }}>
                      <div style={{ fontFamily: 'var(--sidebar-font)', fontSize: 11, color: 'var(--accent)', marginBottom: 8, fontWeight: 700 }}>{agent.agent_name as string}</div>
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        {[0, 1, 2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--fg3)', animation: `pulse 1.4s ease-in-out ${i * 0.2}s infinite` }} />)}
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 16px', display: 'flex', gap: 10, alignItems: 'flex-end' }}>
                <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder={`Tell ${agent.agent_name as string} what to do...`} rows={2} style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontFamily: 'var(--sans)', fontSize: 14, color: 'var(--fg)', resize: 'none', lineHeight: 1.6 }} />
                <button onClick={sendMessage} disabled={running || !input.trim()} className="btn btn-accent" style={{ fontSize: 13, padding: '0 20px', flexShrink: 0, opacity: (running || !input.trim()) ? 0.5 : 1, fontFamily: 'var(--sidebar-font)', fontWeight: 600 }}>Send →</button>
              </div>
              <p style={{ fontFamily: 'var(--sidebar-font)', fontSize: 12, color: 'var(--fg3)', textAlign: 'center', marginTop: 8 }}>Enter to send · Shift+Enter for new line</p>
            </div>
          )}
            ...
            <div className="calendar-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24 }}>
              <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', minWidth: 0 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid var(--border)' }}>
                  {dayNames.map(d => <div key={d} style={{ padding: '10px 0', textAlign: 'center', fontFamily: 'var(--sidebar-font)', fontSize: 11, color: 'var(--fg3)', fontWeight: 600, letterSpacing: 0.5 }}>{d}</div>)}
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
                  {Array.from({ length: getFirstDayOfMonth(calendarDate) }).map((_, i) => <div key={`e-${i}`} style={{ minHeight: 88, borderRight: '1px solid var(--border)', borderBottom: '1px solid var(--border)', background: 'var(--bg3)', opacity: 0.4 }} />)}
                  {Array.from({ length: getDaysInMonth(calendarDate) }).map((_, i) => {
                    ...
                    return (
                      <div key={day} style={{ minHeight: 88, padding: 8, borderRight: colIndex === 6 ? 'none' : '1px solid var(--border)', borderBottom: '1px solid var(--border)', background: isToday ? 'rgba(200,241,53,0.04)' : 'transparent' }}>
                        <div style={{ width: 26, height: 26, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--sidebar-font)', fontSize: 12, fontWeight: isToday ? 700 : 500, background: isToday ? 'var(--accent)' : 'transparent', color: isToday ? '#0a0a0a' : 'var(--fg)', marginBottom: 4 }}>{day}</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          {events.slice(0, 2).map(event => <div key={event.id} style={{ padding: '2px 6px', borderRadius: 4, fontSize: 10, background: `${EVENT_COLORS[event.event_type] || '#6b7280'}22`, color: EVENT_COLORS[event.event_type] || '#6b7280', border: `1px solid ${EVENT_COLORS[event.event_type] || '#6b7280'}44`, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'var(--sidebar-font)', fontWeight: 500 }}>{event.title}</div>)}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      ...
                      {upcomingEvents.map(event => (
                        <div key={event.id} style={{ padding: '12px 14px', background: 'var(--bg3)', borderRadius: 8, border: '1px solid var(--border)', borderLeft: `3px solid ${EVENT_COLORS[event.event_type] || '#6b7280'}` }}>
                          ...
                          <div style={{ marginTop: 6 }}>
                            <span style={{ fontFamily: 'var(--sidebar-font)', fontSize: 10, padding: '2px 8px', borderRadius: 4, background: `${EVENT_COLORS[event.event_type] || '#6b7280'}22`, color: EVENT_COLORS[event.event_type] || '#6b7280', fontWeight: 600 }}>{event.event_type}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
                  ...
                  <p style={{ fontSize: 13, color: 'var(--fg3)', lineHeight: 1.6, marginBottom: 12, fontFamily: 'var(--sidebar-font)' }}>Tell your agent about any meeting or appointment in the chat.</p>
                  <button onClick={() => setView('chat')} className="btn btn-accent" style={{ width: '100%', fontSize: 13, fontFamily: 'var(--sidebar-font)', fontWeight: 600 }}>Open chat →</button>
                </div>
              </div>
            </div>
          )}
